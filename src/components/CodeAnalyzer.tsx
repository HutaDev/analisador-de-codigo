...
const AnalisadorCodigo: React.FC<AnalisadorCodigoProps> = ({ 
  ...
  const iniciarAnalise = async () => {
    if (!codigo.trim()) {
      toast({
        title: "Código vazio",
        description: "Por favor, insira algum código para analisar",
        variant: "destructive",
      });
      return;
    }
    ...
    setTimeout(() => {
      try {
        ...
        toast({
          ...
          description: `Encontrados ${resultado.issues.length} problemas com pontuação de ${resultado.score}/100`,
        });
      } catch (error) {
        toast({
          title: "Falha na análise",
          description: "Ocorreu um erro ao analisar seu código",
          variant: "destructive",
        });
